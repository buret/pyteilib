#! /usr/bin/env python
# -*- coding: utf-8 -*-

"""! @package pyteilib The main package for TEI library support for Python. Normally used by importing the package, and perhaps a particular module inside it.
    
    Usage:
    >>> import pyteilib
    >>> from pyteilib import reader
    >>> pyteilib.tei_read(...)
"""

# Add pyteilib/ folder to path
import sys
sys.path.append(sys.path[-1] + './pyteilib')

sys.path.append('./pyteilib')
sys.path.append('./pyteilib/reader')
sys.path.append('./pyteilib/tei')
sys.path.append('./pyteilib/transformer')
sys.path.append('./pyteilib/writer')

# Initialize API
from wrapper import *

## Functions used to initialize LMF library
def parse_options():
    """! @brief Get command line arguments.
    @return Parsed command line arguments.
    """
    # Options management
    from optparse import OptionParser
    parser = OptionParser()
    parser.add_option("-l", "--log-filename", dest="log_filename", action="store", default=None, help="log filename [default=None]")
    parser.add_option("-v", "--verbose", dest="verbose", action="store_true", default=False, help="print more details to stdout [default=False]")
    parser.add_option("-i", "--uid", dest="uid", action="store_true", default=False, help="include UID in output format [default=False]")
    return parser.parse_args()[0]

## Initialize TEI library global variables
# If a TEI module needs to access options, copy following lines:
# from pyteilib import options
# global options
options = parse_options()
# Initialize helper functions and log options
log("Command line options: " + str(options), options)
