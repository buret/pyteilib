...
for root, dirs, files in os.walk(INPUT_DIR + "xml_pcl_v9/"):
    for file in files:
        if file.endswith(".xml"):
            # Read ParCoLab v9 XML
            tei_pcl_v9 = pyteilib.tei_read(os.path.join(root, file), xml_type='PCLv9')
            # Transform it to BaTelÒc XML
            tei_bto = pyteilib.transform(tei_pcl_v9, from_type='PCLv9', to_type='BTO')
            # Write it to BaTelÒc XML
            pyteilib.tei_write(tei_bto, OUTPUT_DIR + "xml_bto/" + file, xml_type='BTO')
            # Export metadata to v9 format
            pyteilib.export_header(tei_pcl_v9, OUTPUT_DIR + "csv/metadata_v9_to_bto.csv")
